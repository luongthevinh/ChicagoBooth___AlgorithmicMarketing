...
```{r}
open_probs <- predict(opens_model, newdata = all_combinations, response='prob')
...
```{r}
click_probs <- predict(clicks_model, newdata = all_combinations, response = 'prob')

summary(click_probs)
```

The message with the highest predicted probability of being clicked through has the following configuration:
