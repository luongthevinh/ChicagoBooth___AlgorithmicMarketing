...
Third, as shown in the figure above, the lower the looks are rated for women (sender), the more likely are them to send a message to men (receiver) with looks rated below 9 compared to the whole sample. This finding is totally different to men senders with lower rated looks, who behave similar to the whole sample (for women (receiver) looks rated below 9 (shown previously)). 
Specifically, women rated 3 or less (red line) are more likely to send messages overall compared to the whole sample. The same happens for women rated 5 or less (green line), but with a slightly lower magnitude compared to woemn rated 3 or less.
...
#     And compare to the full sample
#     All Sender=Male 
plot(x=1:11,y=pred.prob.male(1:11),type='b',pch=19,col="steelblue",xlab="Receiver Looks",ylab="Predicted Probability", ylim=c(0.03, 0.25), main="Male Senders Likelihood of Sending a Message")
#     for Looks < 6 
lines(x=1:11,y=pred.prob.m11(1:11),type='b',pch=19,col='blue',lty=2)
legend(2,.20, c("All", "Men Rated 11"), lty=c(2,2), col=c("steelblue", "blue"))
...
#     And compare to the full sample
#     All Sender=Male 
plot(x=1:11,y=pred.prob.female(1:11),type='b',pch=19,col="steelblue",xlab="Receiver Looks",ylab="Predicted Probability", ylim=c(0, 0.15), main="Female Senders Likelihood of Sending a Message")
#     for Looks < 6 
lines(x=1:11,y=pred.prob.f11(1:11),type='b',pch=19,col='blue',lty=2)
legend(2,.14, c("All", "Women Rated 11"), lty=2, col=c("steelblue", "blue"))
```

As shown in the figure above, males with looks of 11 are less likely to send a message to women with looks under 10, but much likely to send a message to females with 10 or 11, maybe because of more confidence.
On the other hand, women with looks of 11, are much less likely to send a message to males
than the average except for males with rating of 10. This might be that women prefer good looking mens but not as good looking as them.
